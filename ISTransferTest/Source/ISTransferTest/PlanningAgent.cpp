...
void APlanningAgent::IncreaseEnemyAggression()
{
	...
	m_enemyAggression = FMath::Clamp<float>(m_enemyAggression, 0, 1);
}
...
{
	int frustIndex = -1;
	float frustVal = m_thresholdPortions[0];

	for (int i = 0; i < 4; i++)
	{
		if (m_thresholdPortions[i] < frustVal)
		{
			frustVal = m_thresholdPortions[i];
			frustIndex = i;
		}
	}
